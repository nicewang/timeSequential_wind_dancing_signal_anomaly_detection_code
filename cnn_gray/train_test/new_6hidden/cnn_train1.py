...
def inference(input_tensor, train, regularizer):
    ...
    with tf.variable_scope('layer6-fc2'):
        fc22_weights = tf.get_variable("weight", [512, 512],
                                      initializer=tf.truncated_normal_initializer(stddev=0.1))
        tf.summary.histogram("layer6-fc2/weights", fc22_weights)
        if regularizer != None: tf.add_to_collection('losses', regularizer(fc22_weights))
        fc22_biases = tf.get_variable("bias", [512], initializer=tf.constant_initializer(0.1))
        tf.summary.histogram("layer6-fc2/biases", fc22_biases)

        fc22 = tf.nn.relu(tf.matmul(fc2, fc22_weights) + fc22_biases)
        if train: fc22 = tf.nn.dropout(fc22, 0.5)

    with tf.variable_scope('layer7-fc3'):
        fc32_weights = tf.get_variable("weight", [512, 512],
                                      initializer=tf.truncated_normal_initializer(stddev=0.1))
        tf.summary.histogram("layer7-fc3/weights", fc32_weights)
        if regularizer != None: tf.add_to_collection('losses', regularizer(fc32_weights))
        fc32_biases = tf.get_variable("bias", [512], initializer=tf.constant_initializer(0.1))
        tf.summary.histogram("layer7-fc3/biases", fc32_biases)

        fc32 = tf.nn.relu(tf.matmul(fc22, fc32_weights) + fc32_biases)
        if train: fc32 = tf.nn.dropout(fc32, 0.5)

    with tf.variable_scope('layer8-fc4'):
        fc3_weights = tf.get_variable("weight", [512, 256],
                                      initializer=tf.truncated_normal_initializer(stddev=0.1))
        tf.summary.histogram("layer8-fc4/weights", fc3_weights)
        if regularizer != None: tf.add_to_collection('losses', regularizer(fc3_weights))
        fc3_biases = tf.get_variable("bias", [256], initializer=tf.constant_initializer(0.1))
        tf.summary.histogram("layer8-fc4/biases", fc3_biases)

        fc3 = tf.nn.relu(tf.matmul(fc32, fc3_weights) + fc3_biases)
        if train: fc3 = tf.nn.dropout(fc3, 0.5)

    with tf.variable_scope('layer9-fc5'):
        fc4_weights = tf.get_variable("weight", [256, 256],
                                      initializer=tf.truncated_normal_initializer(stddev=0.1))
        tf.summary.histogram("layer9-fc5/weights", fc4_weights)
        if regularizer != None: tf.add_to_collection('losses', regularizer(fc4_weights))
        fc4_biases = tf.get_variable("bias", [256], initializer=tf.constant_initializer(0.1))
        tf.summary.histogram("layer9-fc5/biases", fc4_biases)

        fc4 = tf.nn.relu(tf.matmul(fc3, fc4_weights) + fc4_biases)
        if train: fc4 = tf.nn.dropout(fc4, 0.5)

    with tf.variable_scope('layer10-fc6'):
        fc5_weights = tf.get_variable("weight", [256, 128],
                                      initializer=tf.truncated_normal_initializer(stddev=0.1))
        tf.summary.histogram("layer10-fc6/weights", fc5_weights)
        if regularizer != None: tf.add_to_collection('losses', regularizer(fc5_weights))
        fc5_biases = tf.get_variable("bias", [128], initializer=tf.constant_initializer(0.1))
        tf.summary.histogram("layer10-fc6/biases", fc5_biases)

        fc5 = tf.nn.relu(tf.matmul(fc4, fc5_weights) + fc5_biases)
        if train: fc5 = tf.nn.dropout(fc5, 0.5)

    with tf.variable_scope('layer11-fc7'):
        fc6_weights = tf.get_variable("weight", [128, 2],
                                      initializer=tf.truncated_normal_initializer(stddev=0.1))
        tf.summary.histogram("layer11-fc7/weights", fc6_weights)
        if regularizer != None: tf.add_to_collection('losses', regularizer(fc6_weights))
        fc6_biases = tf.get_variable("bias", [2], initializer=tf.constant_initializer(0.1))
        tf.summary.histogram("layer11-fc7/biases", fc6_biases)
