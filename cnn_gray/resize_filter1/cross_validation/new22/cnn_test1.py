...
data = []
data_tmp = np.genfromtxt("validate_set/training data.txt")
...
labels = np.genfromtxt("validate_set/labels.txt")
label = labels.reshape(labels.shape[0],)
# print(data.shape)
# print(label.shape)
...
with tf.Session() as sess:
    ...
    # if os.path.exists('validate_acc/test acc in every step training model.txt'):
    #     data_tmp = np.genfromtxt('validate_acc/test acc in every step training model.txt')
    ...
    #     np.savetxt('validate_acc/test acc in every step training model.txt', data)
    # else:
    #     data = np.array([count/label.shape[0]],dtype=np.float64)
    #     np.savetxt('validate_acc/test acc in every step training model.txt', data)
    # if os.path.exists('validate_acc/test abnormal acc in every step training model.txt'):
    #     data_tmp = np.genfromtxt('validate_acc/test abnormal acc in every step training model.txt')
    #     data_tmp = np.array([data_tmp])
    #     if data_tmp.shape != (1,):
    #         data_tmp = data_tmp.reshape(data_tmp.shape[0]*data_tmp.shape[1],1)
    #     data_i = np.array([[count_abnormal/total_abnormal]],dtype=np.float64)
    #     data = np.row_stack((data_tmp, data_i))
    #     np.savetxt('validate_acc/test abnormal acc in every step training model.txt', data)
    # else:
    #     data = np.array([count_abnormal/total_abnormal],dtype=np.float64)
    #     np.savetxt('validate_acc/test abnormal acc in every step training model.txt', data)
    if os.path.exists('validate_acc/count.txt'):
        data_tmp = np.genfromtxt('validate_acc/count.txt')
        data_tmp = np.array([data_tmp])
        if data_tmp.shape != (1,):
            data_tmp = data_tmp.reshape(data_tmp.shape[0]*data_tmp.shape[1],1)
        data_i = np.array([[count]])
        data = np.row_stack((data_tmp, data_i))
        np.savetxt('validate_acc/count.txt', data)
    else:
        data = np.array([count])
        np.savetxt('validate_acc/count.txt', data)
    if os.path.exists('validate_acc/count_abnormal.txt'):
        data_tmp = np.genfromtxt('validate_acc/count_abnormal.txt')
        data_tmp = np.array([data_tmp])
        if data_tmp.shape != (1,):
            data_tmp = data_tmp.reshape(data_tmp.shape[0]*data_tmp.shape[1],1)
        data_i = np.array([[count_abnormal]])
        data = np.row_stack((data_tmp, data_i))
        np.savetxt('validate_acc/count_abnormal.txt', data)
    else:
