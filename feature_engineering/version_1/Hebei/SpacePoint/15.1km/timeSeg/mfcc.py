...
for i in np.arange(files.shape[0]):
    ...
    pl.subplot(8,3,i_+1)
    if i_ == 14 or i_ == 16:
        pl.plot(ceps, color='red')
    elif i_ == 13 or i_ == 15 or i_ == 17 or i_ == 20:
        pl.plot(ceps, color='orange')
    elif i_ == 7:
        pl.plot(ceps, color='green')
    else:
        pl.plot(ceps, color='blue')
    pl.title(title[int(sys.argv[1])]+u" MFCC Coefficient of Diff Data within "+str(i_)+u"-"+str(i_+1)+" (Point at 15.1km)")

pl.subplots_adjust(hspace=0.6)
pl.savefig(title[int(sys.argv[1])]+u" MFCC Coefficient of Diff Data of Point at 15.1km.jpg")
