...
for i in np.arange(files.shape[0]):
    ...
    pl.subplot(8,3,i_+1)
    if i_ == 14 or i_ == 16:
        pl.plot(freqs, np.abs(xf), color='red')
    elif i_ == 13 or i_ == 15 or i_ == 17 or i_ == 20:
        pl.plot(freqs, np.abs(xf), color='orange')
    elif i_ == 7:
        pl.plot(freqs, np.abs(xf), color='green')
    else:
        pl.plot(freqs, np.abs(xf), color='blue')
    pl.xlabel(u"Frequency / Hz")
    pl.ylabel(u"Amplitude / V")
    pl.title(u"Freq Plot within "+str(i_)+u"-"+str(i_+1)+" (Point at 15.1km)")

pl.subplots_adjust(hspace=0.6)
pl.savefig(u"Freq Plot of Point at 15.1km.jpg")
