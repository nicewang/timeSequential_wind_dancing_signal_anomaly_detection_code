...
for i in np.arange(files.shape[0]):
    ...
    elif i_ == 10 or i_ == 13 or i_ == 15 or i_ == 16 or i_ == 19 or i_ == 20:
        pl.plot(ceps, color='orange')
    else:
        pl.plot(ceps, color='blue')
    pl.title(title[int(sys.argv[1])]+u" MFCC Coefficient of Diff Data within "+str(i_)+u"-"+str(i_+1)+" (Point at 18.1km)")

pl.subplots_adjust(hspace=0.6)
pl.savefig(title[int(sys.argv[1])]+u" MFCC Coefficient of Diff Data of Point at 18.1km.jpg")
