...
for i in np.arange(files.shape[0]):
    ...
    pl.title(title[int(sys.argv[1])]+u" MFCC Coefficient of Diff Data within "+str(i_)+u"-"+str(i_+1)+" (Point at 12.1km)")

pl.subplots_adjust(hspace=0.6)
pl.savefig(title[int(sys.argv[1])]+u" MFCC Coefficient of Diff Data of Point at 12.1km.jpg")
