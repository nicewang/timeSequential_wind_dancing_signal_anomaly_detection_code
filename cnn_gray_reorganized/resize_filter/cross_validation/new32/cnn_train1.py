...
def inference(input_tensor, train, regularizer):
    ...
    with tf.variable_scope('layer6-fc2'):
        fc22_weights = tf.get_variable("weight", [512, 256],
                                      initializer=tf.truncated_normal_initializer(stddev=0.1))
        ...
        fc22_biases = tf.get_variable("bias", [256], initializer=tf.constant_initializer(0.1))
    ...
    with tf.variable_scope('layer7-fc3'):
        fc3_weights = tf.get_variable("weight", [256, 2],
                                      initializer=tf.truncated_normal_initializer(stddev=0.1))
        tf.summary.histogram("layer7-fc3/weights", fc3_weights)
        if regularizer != None: tf.add_to_collection('losses', regularizer(fc3_weights))
        fc3_biases = tf.get_variable("bias", [2], initializer=tf.constant_initializer(0.1))
        tf.summary.histogram("layer7-fc3/biases", fc3_biases)
        logit = tf.matmul(fc22, fc3_weights) + fc3_biases
