...
data0 = np.genfromtxt("training_fr_timeFreq_new_1/training data.txt")
label = np.genfromtxt("training_fr_timeFreq_new_1/labels.txt")
data = []
for i in range(data0.shape[0]):
    data_tmp = data0[i,:]
    data_tmp = data_tmp.reshape(10,25)
    b = np.zeros((10, 25, 1))
    for i1 in range(10):
        for j1 in range(25):
...
def inference(input_tensor, train, regularizer):
    ...
    with tf.variable_scope("layer5-conv3"):
        conv3_weights = tf.get_variable("weight", [2, 2, 256, 512],
                                        initializer=tf.truncated_normal_initializer(stddev=0.1))
        # 可视化观看变量
    ...
    with tf.variable_scope('layer7-fc1'):
        fc1_weights = tf.get_variable("weight", [nodes, 1024],
                                      initializer=tf.truncated_normal_initializer(stddev=0.1))
        tf.summary.histogram("layer7-fc1/weights", fc1_weights)
        if regularizer != None: tf.add_to_collection('losses', regularizer(fc1_weights))
        fc1_biases = tf.get_variable("bias", [1024], initializer=tf.constant_initializer(0.1))
        tf.summary.histogram("layer7-fc1/biases", fc1_biases)
    ...
    with tf.variable_scope('layer8-fc2'):
        fc2_weights = tf.get_variable("weight", [1024, 512],
                                      initializer=tf.truncated_normal_initializer(stddev=0.1))
        tf.summary.histogram("layer8-fc2/weights", fc2_weights)
        if regularizer != None: tf.add_to_collection('losses', regularizer(fc2_weights))
        fc2_biases = tf.get_variable("bias", [512], initializer=tf.constant_initializer(0.1))
        tf.summary.histogram("layer8-fc2/biases", fc2_biases)
    ...
    with tf.variable_scope('layer9-fc3'):
        fc3_weights = tf.get_variable("weight", [512, 2],
                                      initializer=tf.truncated_normal_initializer(stddev=0.1))
        tf.summary.histogram("layer9-fc3/weights", fc3_weights)
        if regularizer != None: tf.add_to_collection('losses', regularizer(fc3_weights))
        fc3_biases = tf.get_variable("bias", [2], initializer=tf.constant_initializer(0.1))
        tf.summary.histogram("layer9-fc3/biases", fc3_biases)
